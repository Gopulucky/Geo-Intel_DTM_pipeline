  ...
  return (
    <div className="min-h-screen bg-surface-darker relative overflow-hidden flex flex-col items-center justify-center px-4 py-12">
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden z-0 pointer-events-none">
        <div className="absolute top-[20%] right-[-10%] w-[50%] h-[50%] rounded-full bg-brand-500/10 blur-[150px] animate-pulse-slow"></div>
        <div className="absolute bottom-[-20%] left-[-10%] w-[50%] h-[50%] rounded-full bg-blue-500/10 blur-[150px] animate-pulse-slow" style={{ animationDelay: '2s' }}></div>
      </div>

      <div className="relative z-10 w-full max-w-5xl flex flex-col lg:flex-row gap-6">
        
        {/* Left Side: Progress & Stages */}
        <div className="flex-1 glass-panel-dark rounded-3xl p-8 flex flex-col relative overflow-hidden shadow-2xl border-t border-white/10">
            {/* Glossy top edge */}
            ...
            <div className="flex items-center gap-4 mb-6">
               ...
               <div>
                  ...
                  <p className="text-slate-400 text-sm mt-1 flex items-center gap-2">
                    <span className="font-mono bg-slate-800 px-2 py-0.5 rounded text-brand-300">ID: {jobId}</span>
                    <span className="w-1.5 h-1.5 rounded-full bg-slate-600"></span>
                    <span className="text-white font-medium">{job?.village || "Initializing..."}</span>
                  </p>
               </div>
            </div>
            ...
            <div className="mb-8 relative">
              ...
              <div className="bg-slate-800/80 rounded-full h-3 mb-2 overflow-hidden border border-slate-700/50 p-0.5 shadow-inner">
                <div
                  className="bg-gradient-to-r from-brand-600 via-emerald-400 to-brand-300 h-full rounded-full transition-all duration-1000 ease-out relative"
            ...
            <div className="space-y-4 flex-1">
              {STAGES.map((stage) => {
                ...
                return (
                  <div key={stage.id} className={`flex items-start gap-4 p-4 rounded-2xl transition-all duration-500 ${
                    isCurrent ? "bg-brand-500/10 border border-brand-500/30 shadow-[0_0_20px_rgba(34,197,94,0.1)] translate-x-2" :
                    isDone    ? "bg-slate-800/40 border border-transparent opacity-80" : 
                    ...
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${
                      ...
                      {isDone ? <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg> : 
                       isCurrent ? <span className="animate-spin text-xl">⏳</span> : 
                       <span className="font-mono text-sm">{stage.id}</span>}
                    </div>
                    <div>
                      <p className={`font-semibold font-display tracking-wide ${isCurrent ? "text-brand-300" : "text-slate-200"}`}>
                        {stage.label}
                      </p>
                      <p className="text-sm text-slate-400 mt-0.5">{stage.desc}</p>
                    </div>
                  </div>
                );
              })}
            ...
            {/* Errors */}
            {job?.errors?.length > 0 && (
              <div className="mt-6 bg-red-500/10 border border-red-500/20 rounded-2xl p-5 text-left animate-slide-up">
            ...
            {(job?.status === "complete" || job?.status === "partial") && (
              <div className="mt-6 bg-brand-500/10 border border-brand-500/30 rounded-2xl p-5 text-center shadow-[0_0_30px_rgba(34,197,94,0.15)] animate-zoom-in">
                <p className="font-bold text-brand-300 flex justify-center items-center gap-2 text-lg">
                    <svg className="w-6 h-6 animate-bounce" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                    Pipeline Complete!
                </p>
                <p className="text-sm text-brand-400/70 mt-1">Routing to intelligence dashboard...</p>
              </div>
            )}

            {(job?.status === "failed" || job?.status === "timeout") && (
              <div className="mt-6 bg-red-500/10 border border-red-500/30 rounded-2xl p-5 text-center">
        ...
        <div className="flex-1 lg:max-w-md bg-[#0a0f1a] rounded-3xl p-1 flex flex-col min-h-[500px] border border-slate-800 shadow-2xl relative">
           {/* macOS style terminal header */}
           <div className="bg-slate-900/80 rounded-t-3xl px-4 py-3 flex items-center justify-between border-b border-slate-800">
               <div className="flex gap-2">
                   <div className="w-3 h-3 rounded-full bg-red-500/80"></div>
                   <div className="w-3 h-3 rounded-full bg-yellow-500/80"></div>
                   <div className="w-3 h-3 rounded-full bg-green-500/80"></div>
               </div>
               <div className="text-slate-500 font-mono text-xs font-semibold tracking-wider flex items-center gap-2">
           ...
           <div className="flex-1 overflow-y-auto font-mono text-[13px] p-5 space-y-2 custom-scrollbar bg-transparent leading-relaxed tracking-wide">
